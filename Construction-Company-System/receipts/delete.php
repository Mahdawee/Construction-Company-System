...
$id = (int)($_POST['id'] ?? 0);
$stmt = $pdo->prepare('SELECT id FROM money_receipts WHERE id = :id AND project_id = :pid');
$stmt->execute([':id' => $id, ':pid' => $projectId]);
if ($stmt->fetch()) {
    $pdo->prepare('DELETE FROM money_receipts WHERE id = :id')->execute([':id' => $id]);
    auditLog('delete', 'money_receipts', $id, 'حذف رسید پول');
    flash('success', 'رسید حذف شد.');
} else {
    flash('danger', 'رسید یافت نشد.');
}
