...
$id = (int)($_POST['id'] ?? 0);
$stmt = $pdo->prepare('SELECT id FROM kahta_advances WHERE id = :id AND project_id = :pid');
$stmt->execute([':id' => $id, ':pid' => $projectId]);
if ($stmt->fetch()) {
    $pdo->prepare('DELETE FROM kahta_advances WHERE id = :id')->execute([':id' => $id]);
    auditLog('delete', 'kahta_advances', $id, 'حذف پیش‌پرداخت');
